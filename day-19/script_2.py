...
def main():
    ...
    while x < 1000 and y < 1000:
        ...
        beamer.rb = 0
        beamer.run()
        if beamer.outputs[-1] == 1:
            print((x, y))
            beam_coordinates.append([x, y])
            x -= 1
            fail_count = 0
        elif beamer.outputs[-1] == 0 and fail_count < 5:
            fail_count += 1
            x -= 1
        else:
            print(f'Line {y} has {sum([1 for x in beam_coordinates if x[1] == y])}')
            y += 1
            x = round(y * 0.90)
            fail_count = 0
    print(beam_coordinates[-10:])
