...
def main():
    camera = Intcode_computer()
    camera.read_sequence('input.txt')
    camera.run()
    photo = []
    for i in camera.outputs:
        photo.append(replace_pixel(i))
    photo = "".join(photo)
    photo = photo.split("\n")
    for i in photo:
        print(i)
    alignment = 0
    for i, row in enumerate(photo[1:-3]):
        i += 1
        for j, v in enumerate(row[1:-1]):
            j += 1
            if v == photo[i + 1][j] == photo[i - 1][j] == photo[i][j + 1] == photo[i][j - 1] == "#":
                alignment += i * j
    print(alignment)
