...
def replace_pixel(x):
    switcher = {"35":'#', "46":'.', "10": "\n", "94": "<"}
    return switcher[str(x)]

def main():
    vacuum = Intcode_computer()
    vacuum.read_sequence('input.txt')
    vacuum.sequence[0] = 2    

    input = []
    input.append('A,B,A,A,B,C,B,C,C,B')
    input.append('L,12,R,8,L,6,R,8,L,6')
    input.append('R,8,L,12,L,12,R,8')
    input.append('L,6,R,6,L,12')
    input.append('n')
    for i,inp in enumerate(input):
        input[i] = [int(ord(x)) for x in inp]
        input[i].append(10)
    input = [x for subinput in input for x in subinput]
    vacuum.inputs = input
    vacuum.run()
    print(vacuum.outputs)
    print(vacuum.state)
    print(vacuum.inputs)
