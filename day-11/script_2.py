...
def print_colors(color_dict):
    x = [int(x.strip('][').split(', ')[0]) for x in color_dict.keys()]
    y = [-int(x.strip('][').split(', ')[1]) for x in color_dict.keys()]
    v = [x for x in color_dict.values()]
    min_x = min(x)
    max_x = max(x)
    min_y = min(y)
    max_y = max(y)
    row = [' ' for i in range(0, max_x - min_x + 1)]
    matrix = [row.copy() for i in range(0, max_y - min_y + 1)]
...
async def main():
    ...
    print_colors(hull_robot.colors)
